...
Calling "tf checkout" writes to the error stream if the file is checked out by any other user in any workspace (with error saying "NativeCommandError"). Powershell by default considers any error stream output as an error. Hence, checking out a file using TF from the PowerShell did not work well (The script had 'ErrorActionPreference ="Stop"').   
